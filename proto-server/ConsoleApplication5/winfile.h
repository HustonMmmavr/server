...
public:
	...
	*/
	...
	static void ParseMode(PDWORD, PDWORD, FileOpenMode);
	...
	*/
	static HANDLE Open_TCHAR(const TCHAR *fileName, FileOpenMode mode);
	...
	*/
	...
	\return TRUE if file exists, FALSE in other case.
	*/
	static bool Exist_TCHAR(const TCHAR *wFileName);
	...
	*/
	...
	\param[in] fileName Name of file.
	*/
	static void Delete_TCHAR(const TCHAR *tFileName);
	...
	*/
	...
	//static bool IsUnicodeFormat();
private:
	/*!
	Convert input char filename into char or wchar_t. Depend on compiler define. static.
	\param[in] string fileName - name of file.
	\param[in\out] string tFileName converted string.
	*/
	static void ConvertFileName(const char *fileName, TCHAR *tFileName);
	HANDLE _hFile;			///< Handle of opened file
	char *_fileName;			///< Name of file
	TCHAR *_tFileName;			
	wchar_t *_wFileName;		///< Name of file in unicode charset
